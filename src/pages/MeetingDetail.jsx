...
export default function MeetingDetail() {
  ...
  const toggleTodoDone = async (todo) => {
    ...
    if (newStatus === 'done') {
      ...
      if (!existing) {
        ...
        await supabase.from('np_meeting_todos').update({ type: 'completed' }).eq('id', existing.id);
      }
    }
    loadData();
  };
  ...
  const completedInMeeting = meetingTodos.filter(mt => mt.type === 'completed').map(mt => mt.np_todos).filter(Boolean);
    ...
    <>
      ...
      <div className="content">
        ...
        <div className="section-card">
          ...
          {completedInMeeting.length > 0 && (
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-muted)', marginBottom: 8 }}>Erledigte Aufgaben</div>
              {completedInMeeting.map(t => (
                <div key={t.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 0', fontSize: 13, color: 'var(--text-muted)', textDecoration: 'line-through' }}>
                  ✅ {t.title}
                </div>
              ))}
            </div>
